...
import {getPrincipal, getPrincipalId, getRoleFromPhoto} from "../metier/AuthService";
import {VARIABLE_ENVIRONEMENT} from "../consts/Consts";
import {customGetOnePromiseUsers} from "../metier/UsersService";
...
function Navbar() {
    ...
    const authItems  = () => {
        if(localStorage.getItem("token")){
            ...
            return <>
                ...
                <button onClick={doLogout} style={{marginLeft: '5vh'}} className="btn btn-outline-success" type="submit">Logout</button>
            </>
        }
        else {
            ...
            return (
                <>
                <Link to={'/login'} className="btn btn-outline-success " style={{marginLeft: '90vh'}} type="submit">Login</Link>
    ...
    return (
        <nav className="d-flex  container  mt-3" style={{backgroundColor: 'transparent'}}>
            <div style={{display: 'flex', cursor: 'pointer'}} onClick={()=>navigate('/')}>
                <img src={logo} height={'50'} width={'50'} />
                <Link className="navbar-brand mt-2" to={'/'}>ERK Livres</Link>
            </div>
